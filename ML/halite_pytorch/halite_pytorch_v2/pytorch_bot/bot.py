...
class Bot:
    def __init__(self, location, name):
        ...
        random_input_data = Variable(torch.randn(1,PLANET_MAX_NUM, PER_PLANET_FEATURES))
        predictions = self._neural_net.predict(random_input_data)
        assert len(predictions.data.numpy()[0]) == PLANET_MAX_NUM
        """
    ...
    def play(self):
        ...
        while True:
            ...
            normalized_array = normalize_input(np.array([features]))
            normalized_features = Variable(torch.from_numpy(normalized_array).float())
            #normalized_features = normalize_input(np.array(features))
            predictions = self._neural_net.predict(normalized_features)
